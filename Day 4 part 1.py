passstr ="""eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
answer = 0
count = 0
passlist = passstr.split('\n\n')
total = len(passlist)
Keywords = ["ecl", "eyr", "hcl", "byr", "iyr", "pid", "hgt"]

def passportchecker(passports):
    if all(i in passports for i in Keywords):
        return True
    else:
        return False 

while count < total:
    if passportchecker(passlist[count]) == True:
        answer += 1
    count += 1
